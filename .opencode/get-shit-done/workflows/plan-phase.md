...
@D:/Developer/Minecraft_bot/.opencode/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
...
```bash
PHASE_INFO=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```bash
PHASE_DESC=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `D:/Developer/Minecraft_bot/.opencode/get-shit-done/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
PHASE_SECTION=$(node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
```bash
node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  node "D:/Developer/Minecraft_bot/.opencode/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
fi
```
