...
- **Global scope**: `D:/Developer/Minecraft_bot/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Developer/Minecraft_bot/.opencode/notes/`)
...
2. Glob `D:/Developer/Minecraft_bot/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/Developer/Minecraft_bot/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Developer/Minecraft_bot/.opencode/notes/` — works in any directory
